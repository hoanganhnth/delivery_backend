...
public class MatchEventServiceImpl implements MatchEventService {
    ...
    public MatchEventServiceImpl(KafkaTemplate<String, String> kafkaTemplate, ObjectMapper objectMapper) {
    ...
    public void processShipperMatchResult(FindShipperEvent deliveryEvent, List<NearbyShipperResponse> shippers) {
        ...
        // Log found shippers
        shippers.forEach(shipper -> log.info(
                "🚚 Found shipper at location: {}, {} with distance: {}km for delivery: {}",
    ...
    public void publishMatchFoundEvent(FindShipperEvent deliveryEvent, NearbyShipperResponse shipper) {
        try {
            ...
            String eventJson = objectMapper.writeValueAsString(matchEvent);
            kafkaTemplate.send(KafkaTopicConstants.SHIPPER_MATCHED_TOPIC, eventJson);
    ...
    public void publishNoShipperAvailableEvent(FindShipperEvent deliveryEvent) {
        try {
            ...
            kafkaTemplate.send(KafkaTopicConstants.NO_SHIPPER_AVAILABLE_TOPIC, eventJson);
    ...
    private boolean isValidShipperInfo(NearbyShipperResponse shipper) {
        ...
        // shipper.getShipperName().trim().isEmpty()) {
        // log.warn("⚠️ Shipper name is null/empty for shipper: {}",
        // shipper.getShipperId());
        // return false;
        // }

        // if (shipper.getShipperPhone() == null ||
        // shipper.getShipperPhone().trim().isEmpty()) {
        // log.warn("⚠️ Shipper phone is null/empty for shipper: {}",
        // shipper.getShipperId());
        // return false;
        // }
    ...
    private boolean isValidDeliveryEvent(FindShipperEvent event) {
        ...
        // if (event.getRestaurantName() == null ||
        // event.getRestaurantName().trim().isEmpty()) {
        // log.warn("⚠️ Restaurant name is null/empty for delivery: {}",
        // event.getDeliveryId());
        // return false;
        // }
