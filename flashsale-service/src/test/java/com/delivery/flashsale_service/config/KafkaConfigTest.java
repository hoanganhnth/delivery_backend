...
class KafkaConfigTest {
    ...
    void usesConfiguredConsumerIdentityAndRawRetrySerializer() {
        KafkaConfig config = new KafkaConfig("kafka:19092", "flashsale-test-group", false);
        assertThat(config.consumerFactory().getConfigurationProperties())
                ...
                .containsEntry(ConsumerConfig.GROUP_ID_CONFIG, "flashsale-test-group");
    ...
    void retryTopicFallbackReturnsToTheCanonicalFlashSaleDlt() {
        KafkaConfig config = new KafkaConfig("kafka:19092", "flashsale-test-group", true);
        KafkaTemplate<String, String> template = mock(KafkaTemplate.class);
        when(template.send(any(ProducerRecord.class)))
                .thenReturn(CompletableFuture.completedFuture(mock(SendResult.class)));
        var recoverer = config.flashSaleDeadLetterRecoverer(template);

        recoverer.accept(new ConsumerRecord<>("order.created-retry-flashsale-1000", 2, 3L,
        ...
        assertThat(sent.getValue().topic()).isEqualTo("order.created.flashsale.DLT");
