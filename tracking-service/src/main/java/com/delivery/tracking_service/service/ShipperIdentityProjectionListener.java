...
public class ShipperIdentityProjectionListener {
    ...
    public ShipperIdentityProjectionListener(ObjectMapper mapper, ShipperIdentityProjectionRepository projections,
            ShipperIdentityInboxReceiptRepository receipts) { this.mapper = mapper; this.projections = projections; this.receipts = receipts; }
    @RetryableTopic(
    ...
    public void upsert(String raw) throws Exception {
        ...
        if (!ShipperIdentityUpserted.TYPE.equals(event.eventType()) || event.principalId() == null || event.legacyUserId() == null
                || event.shipperId() == null || event.mappingVersion() < 1) throw new IllegalArgumentException("Invalid shipper identity event");
        String fingerprint = fingerprint(raw); ShipperIdentityInboxReceipt prior = receipts.findById(event.eventId()).orElse(null);
        if (prior != null) { if (!prior.getEventType().equals(event.eventType()) || !prior.getPrincipalId().equals(event.principalId())
                || !prior.getPayloadFingerprint().equals(fingerprint)) throw new IllegalStateException("Conflicting shipper identity event reuse"); return; }
        ShipperIdentityProjection projection = projections.findById(event.principalId()).orElse(null);
        if (projection != null && event.mappingVersion() < projection.getMappingVersion()) { receipt(event, fingerprint); return; }
        if (projection == null) { projection = new ShipperIdentityProjection(); projection.setPrincipalId(event.principalId()); }
        if (projection.getMappingVersion() != null && event.mappingVersion() > projection.getMappingVersion() + 1) throw new IllegalStateException("Shipper identity mapping version gap");
        projection.setLegacyUserId(event.legacyUserId()); projection.setShipperId(event.shipperId()); projection.setMappingVersion(event.mappingVersion()); projection.setUpdatedAt(LocalDateTime.now());
        projections.save(projection); receipt(event, fingerprint);
    }
    private void receipt(ShipperIdentityUpserted event, String fingerprint) { ShipperIdentityInboxReceipt value = new ShipperIdentityInboxReceipt();
        value.setEventId(event.eventId()); value.setEventType(event.eventType()); value.setPrincipalId(event.principalId()); value.setPayloadFingerprint(fingerprint); value.setProcessedAt(LocalDateTime.now()); receipts.save(value); }
    private static String fingerprint(String raw) { try { return java.util.HexFormat.of().formatHex(MessageDigest.getInstance("SHA-256").digest(raw.getBytes(StandardCharsets.UTF_8))); }
