# Delivery platform documentation map

> Canonical cross-system documentation owner: `backend_delivery`. The workspace
> root `docs/` path is a compatibility surface during this migration; do not
> create new platform truth there.
...
- `WORKFLOW.md`: canonical request, planning, judgment, validation, and
  completion behavior.
- `system/`: canonical reconstruction entry point for the Delivery platform:
  architecture, service/API/event/data/security/client/operations documentation,
  source map, diagram standards, clean-room rebuild sequence, and simulator
  design. Start at [`system/README.md`](./system/README.md).
- `system/handbook-catalog.json`: allowlisted public-handbook inputs and
  classification defaults.
- `system/reconciliation-matrix.md`: source authority, proof and freshness
  ledger for cross-repository claims.
- `archive/`: historical material that must not override current source truth.
- `ARCHITECTURE.md`: editable Mermaid source for the current polyrepo,
  identity boundary, critical workflows, data ownership, and client contracts.
