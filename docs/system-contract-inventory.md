...
| delivery-service | 8085 | PostgreSQL `delivery_db`, Kafka; legacy STOMP hidden | exact delivery routes | offer/accept identity + transactional outbox + authenticated self current-offer recovery đã khóa; exact REJECT/cancel-assignment replay trước rematch không phát duplicate event; PostgreSQL/Kafka crash/concurrency proof còn OPEN |
...
| settlement-service | 8090 | PostgreSQL `settlement_db`, Kafka | exact admin GET-only; client/payment/mutation hidden | ledger unique key + balance row lock; `delivery.completed.totalPrice` must equal restaurant earnings + restaurant commission + shipping fee and is the COD debit source; mismatch fails before ledger mutation; transactional MANUAL ACK and replay proof PASS. |
...
| promotion-service | 8096 | PostgreSQL `promotion_db`; Kafka | user wallet + admin campaign; calculate/reservation internal | V2 durable voucher reservation, V3 deterministic outbox, locked wallet/capacity, 15-minute expiry, exact replay/conflict, commit/release and `COMMITTED -> RELEASED` compensation complete; only ADMIN-owned `ALL/SHOP` vouchers are checkout-eligible; checkout/relay flags false by default; PostgreSQL duplicate-use race PASS. |
| analytics-service | 8097 | PostgreSQL `analytics_db`, Kafka | hidden khỏi Gateway cho MVP | REST/listener/reconciliation đều off mặc định; raw events có unique dedup key, listener không ACK lỗi và reconciliation đọc page 500 thay vì tải cả ngày; PostgreSQL race/retry-DLT/ownership proof còn OPEN |
| flashsale-service | 8092 trong container riêng | PostgreSQL, Kafka; Redis non-authoritative | exact public/admin reads; merchant/internal hidden | V2 durable reservation/lines and V3 deterministic outbox; sorted PostgreSQL row locks make multi-line reserve atomic; 15-minute expiry, exact replay, commit/release and compensation complete. Last-stock race and exhausted-second-line rollback PASS; checkout/relay/merchant flags false by default. |
...
|---|---|---|---|
| `order.created` | order | saga, notification, promotion, flashsale; analytics disabled | active; immutable monetary snapshot plus nullable stable voucher/flash reservation identity. Promotion/Flash-sale commit idempotently and ACK only after success. Exact reservation replay is a no-op; contradictory payload fails closed. |
| `order.cancelled` | order | saga, promotion, flashsale, settlement refund boundary (flagged off); analytics disabled | active core cancellation; carries event identity/occurredAt from the transactional outbox, nullable stable voucher/flash reservation identity, and the immutable monetary snapshot (`subtotalPrice`, `discountAmount`, `shippingFee`, `totalPrice`, `paymentMethod`). Customer/admin/restaurant cancellation, restaurant rejection, and payment failure enqueue the same compensation contract. Promotion/Flash-sale release `RESERVED` or pre-fulfilment `COMMITTED` exactly once. Settlement refund processing remains default-off and fail-closed for provider/manual review. |
...
| `saga.command.create-delivery` | saga | delivery | active; Saga transition + stable command outbox commit atomic; Delivery bắt buộc `eventId`, positive `shippingFee`, zero discount, exact `subtotal + shippingFee = totalPrice`, COD và đủ pickup/delivery coordinate hữu hạn trong Việt Nam; lưu unique create-event identity và chỉ skip replay khi toàn bộ ownership/location/COD fields khớp delivery đã tạo |
...
| `saga.command.find-shipper` | saga | match | active; canonical COD payload; `DELIVERY_CREATED` phải có persisted `delivery.created.result`, không dùng restaurant event làm fallback cho delivery/location; per-order ordered transactional outbox; isolated Saga timeout rehearsal giữ location/COD và exclude đúng timed-out shipper, pending outbox phục hồi sau broker bootstrap failure và peer restart không phát duplicate; Match execution trên Redis thật vẫn là gate riêng |
| `shipper.found` | match | saga | active; đúng một shipper đủ deposit; `eventId` được dẫn xuất ổn định từ UUID command + outcome và `matchingSessionId` là command generation, publisher fail-closed nếu thiếu identity thay vì sinh UUID mới; key `orderId`, broker failure được propagate để retry; Match chỉ sở hữu GEO/availability nên không bịa shipper name/phone/rating |
| `shipper.not-found` | match | saga | active; `eventId` được dẫn xuất ổn định từ UUID command + terminal outcome, key `orderId`; Saga phát hai command terminal riêng để Order và Delivery cùng hội tụ `SHIPPER_NOT_FOUND`; Kafka replay còn OPEN |
| `saga.command.cache-shipper-found` | saga | delivery | active; persist offer + expiry trước notification |
| `saga.command.expire-shipper-offer` | saga | delivery | active; timeout command mang exact delivery/shipper/deadline generation, Delivery lock row rồi chỉ clear matching expired offer; PostgreSQL/Kafka two-instance accept-vs-expired-timeout, delayed stale timeout và exact timeout replay PASS; source lag 0, DLT không tăng sau valid replay và offsets giữ nguyên qua broker OOM restart; intentionally early timeout fail-closed vào DLT; isolated Saga scheduler→outbox→Kafka→Delivery timeout/rematch, pending relay recovery và peer restart idempotency PASS |
| `saga.command.mark-shipper-not-found` | saga | delivery | active; stable command `eventId` + positive order/delivery identity; Delivery lock đúng row và chỉ chuyển `FINDING_SHIPPER -> SHIPPER_NOT_FOUND`; tách khỏi cancellation để không còn Delivery=`CANCELLED` trong khi Order=`SHIPPER_NOT_FOUND` |
| `delivery.shipper-offered` | delivery | notification | active; delivery mutation + stable-event outbox commit atomic; payload must carry canonical restaurant/pickup/delivery text (no UI fallback placeholders); Notification requires producer `eventId` and dedups by `eventId + shipperId`, so replay is suppressed while a later rematch to the same shipper remains deliverable; concurrent/runtime replay OPEN |
| `saga.command.stop-matching` | saga | match | active |
| `saga.command.update-order-status` | saga | order | active; stable command `eventId`, positive top-level `orderId`, JSON-object `originalEvent`; inner `orderId` nếu có phải khớp command; `SHIPPER_NOT_FOUND` bắt buộc positive `deliveryId`; Saga enrich persisted delivery identity cho compensation; failure chỉ dùng canonical `CANCELLED`/`SHIPPER_NOT_FOUND`; per-order ordered transactional outbox; Delivery-status rehearsal tạo đúng 3 command, relay/restart giữ Kafka cardinality 3 |
...
| `delivery.completed` | delivery | settlement | active; producer copies immutable Order `totalPrice`. Consumer requires `totalPrice = restaurantEarnings + restaurantCommission + shippingFee` and `shippingFee = shipperEarnings + shippingCommission`; COD debit uses that same total. Stable receipt/fingerprint, transactional ledger, ACK-after-commit, exact replay, concurrent duplicate and crash recovery proof remain PASS. |
...
- Custom Kafka config của order, delivery, match, saga và notification đã đọc
  `spring.kafka.bootstrap-servers`; Compose override được unit/render proof.
- Saga, notification và Order không còn ack parse/service failure; retry hai lần
  ...
  Notification commit source offset sau publish thành công, tránh poison bị DLT
...
- Match command listener phân biệt `NoShipperAvailableException` với lỗi hạ tầng:
  chỉ hết candidate sau business retry mới phát `shipper.not-found`. Listener
  không còn tự `subscribe()` rồi giữ acknowledgment sau khi method return;
  Spring Kafka quản lý Mono completion với async ack, còn lỗi hạ tầng đi qua
  retry topics và DLT. Output ID nay deterministic theo command generation;
  publisher không còn sinh random ID lúc replay và GEO projection không tạo
  profile/rating giả. Clean Match 48 executed + 2 opt-in Redis test skipped;
  restart/order/DLT proof bằng Kafka thật vẫn OPEN.
...
- Match location/status Redis replica consumers chỉ ACK sau mutation và chuyển lỗi
  ...
  Cancellation tombstone theo delivery là monotonic: delayed find command không
  còn clear flag và resurrect matching. Offer ownership dùng cặp Redis key
  `shipper → delivery`/`delivery → shipper`; reserve kiểm tombstone atomically,
  còn `stop-matching` ghi tombstone trước rồi exact-generation release cả hai
  chiều. Redis read/write failure fail-closed vào Kafka retry. Dead
  `stopMatchingProcess` wrapper không call site và từng nuốt lỗi đã xóa. Redis
  serializer proof cho reserve/cancel/stale-release đạt 2/2; Kafka replay runtime
  tổng vẫn OPEN.
...
- Authenticated shipper-offer recovery boundary: Delivery expose exact self-only
  `GET /api/deliveries/offers/current` qua Gateway role `SHIPPER`, derive identity
  từ trusted JWT header và chỉ trả offer chưa hết hạn; nhiều offer đồng thời fail
...
- Search immutable replay closeout: per-entity checkpoint trước đây chỉ giữ
  occurredAt/eventId/action nên cùng eventId nhưng payload khác vẫn có thể reapply
  vào Elasticsearch. Checkpoint nay giữ canonical SHA-256 payload fingerprint;
  exact retry cùng metadata/payload vẫn reapply document sau crash, contradictory
  same-ID metadata/payload fail-closed, stale event ID vẫn bị fence. Checkpoint cũ
  thiếu fingerprint chỉ được upgrade khi eventId/occurredAt/action khớp. Clean
  Search 9/9 và Restaurant producer 1/1; live multi-instance/partition, index
  refresh và Elasticsearch recovery proof vẫn OPEN ở Gate B8.
