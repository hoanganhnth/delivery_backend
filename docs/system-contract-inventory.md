...
| auth-service | 8081 | PostgreSQL `auth_db` | `/api/auth/**` | `USER_SERVICE_URL` đã wire; admin block/unblock dùng account row lock transaction, revoke active sessions và durable pending marker trước commit; User projection sync chạy after-commit/scheduled retry với version guard; external key paths + private/public pair preflight có focused proof; mounted-key container startup + Auth V2 PostgreSQL schema proof PASS |
...
| order-service | 8084 | PostgreSQL `order_db`, Kafka; HTTP restaurant/promotion/flash sale | exact create/read/cancel + admin paths | create/cancel transactional outbox có H2 proof; read/cancel actor checks require both trusted `X-User-Id` and the matching role (`USER`, `SHOP_OWNER`, assigned `SHIPPER` read-only, or `ADMIN`) so missing/wrong role direct calls fail closed; public cancel dùng row lock, set cancellation timestamp trước outbox và exact retry cùng actor/reason là no-op không phát thêm event; internal restaurant-decision eligibility dùng order row lock trước pending check để không đọc stale PENDING khi cancel transaction đang chạy; create-order request idempotency key chưa thuộc contract hiện tại và vẫn OPEN cho client/backlog; PostgreSQL/Kafka replay còn OPEN |
...
| flashsale-service | 8092 trong container riêng | PostgreSQL, Redis, Kafka | exact public/admin reads; merchant/internal hidden | checkout/merchant flags off; PostgreSQL/Redis runtime proof còn OPEN. Checkpoint 2026-07-29: Gateway không route internal reserve/merchant item; internal reserve bắt `Internal-Token` trước checkout flag/validation, `FlashSaleStockService`/RedisConfig và merchant controller không tạo bean mặc định. Flashsale `24/24`, Gateway route-security `13/13` PASS |
...
- Auth/Gateway runtime matrix xác nhận access/refresh TTL 900/604800 giây;
  ...
  không leo ADMIN và cross-user address bị 403. Refresh rotation/logout revoke
  server-side refresh đúng; access JWT cũ vẫn hợp lệ tới TTL 15 phút vì Gateway
...
- Auth account/session schema đã chuyển sang Flyway V1 + Hibernate `validate`.
  Migration giữ legacy rows, fail-closed khi thiếu table/cột, duplicate email hoặc
  duplicate non-null refresh token; DB uniqueness giờ khớp repository contract
  `Optional` + pessimistic refresh-token lock. Active-expiry và device-session
  indexes được tạo rõ ràng. Auth clean 44/44; PostgreSQL concurrent registration/
  rotation và token TTL runtime proof vẫn thuộc B8.
