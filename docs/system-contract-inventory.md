...
| settlement-service | 8090 | PostgreSQL `settlement_db`, Kafka | exact admin GET-only, including read-only refund queue; client/payment/mutation hidden | ledger unique key + balance row lock; `delivery.completed.totalPrice` must equal restaurant earnings + restaurant commission + shipping fee and is the COD debit source; mismatch fails before ledger mutation; cancellation refund cases keep immutable monetary snapshots and idempotency, while listener/outbox/provider execution remain default-off; transactional MANUAL ACK and replay proof PASS. |
