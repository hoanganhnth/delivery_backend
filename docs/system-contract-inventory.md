...
| `order.cancelled` | order | saga, promotion, flashsale; analytics disabled | active core cancellation; carries nullable stable voucher/flash reservation identity. Customer/admin/restaurant cancellation, restaurant rejection, and payment failure enqueue the same compensation contract. Promotion/Flash-sale release `RESERVED` or pre-fulfilment `COMMITTED` exactly once. |
