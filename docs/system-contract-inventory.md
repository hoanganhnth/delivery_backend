...
| restaurant-service | 8083 | PostgreSQL `restaurant_db`, Kafka | `/api/restaurants/**`, `/api/menu-items/**` | confirm/reject decision+outbox atomic; direct restaurant decision publisher now fails closed on invalid order/restaurant/actor/prep-time/rejection input before touching eligibility or outbox; PostgreSQL two-instance race, broker-down/backoff, relay restart và Kafka cardinality PASS; checkout reads canonical `openingHour/closingHour`, accepts only menu status `AVAILABLE`, and requires menu ownership plus finite positive canonical price; rating duplicate sequential/constraint race maps to 409 conflict with no average rewrite after duplicate; additive serviceability zones use validated WGS84 polygons and internal decision response, gated by `RESTAURANT_SERVICEABILITY_ENABLED=false`; additive menu inventory ledger/reservation owns `on_hand - reserved`, sorted row locks, 15-minute hold, commit/release/expiry and replay-safe order-event receipt, gated by `RESTAURANT_INVENTORY_ENABLED=false`; Restaurant `112/112`, Order consumer `87/87`; inventory race/runtime and downstream Order crash-window còn OPEN |
| routing-service | 8094 | provider boundary, no business DB | internal `/internal/routing/v1/**` | owns driving route/matrix and additive ETA window (`driving + prep`, upper bound +10 minutes), returns explicit provider/fallback source, shared internal credential and no Gateway route; Mapbox/runtime rehearsal remains deferred |
| order-service | 8084 | PostgreSQL `order_db`, Kafka; HTTP restaurant/promotion/flash sale | exact create/preview/read/cancel + admin paths | server-owned regular/flash/voucher pricing; stacking permits one `SHOP_DISCOUNT`, one `PLATFORM_DISCOUNT`, and one `FREESHIP` layer, while voucher and flash-sale lines do not stack; immutable monetary/reservation snapshot now includes additive `inventoryReservationId`; synchronous same-identity release on ambiguous/later create failure; cancellation/payment failure emits compensation IDs through transactional outbox. When `ORDER_INVENTORY_RESERVATION_ENABLED=false` no inventory call is made; when enabled Order reserves/commits through Restaurant and fails closed on unavailable/malformed responses. `saga.command.update-order-status` has a durable `saga_command_receipts` inbox: an atomic PostgreSQL `INSERT .. ON CONFLICT DO NOTHING` claim commits with the Order mutation before listener ACK; exact raw-payload replay ACKs without another mutation, while a reused ID with changed command/order/status/payload fails closed. The shared Kafka factory preserves raw `String` commands for the SHA-256 receipt, while DTO listeners still deserialize JSON; its recoverer also uses the raw-string template so DLT does not JSON-quote a command. Two-replica Kafka/PostgreSQL and fresh-group replay proof PASS. All Order retry/DLT and common-error-handler fallback destinations are owner-isolated as `-retry-order-*` / `.order.DLT` during the shared-source migration. Stacking and inventory flags remain false by default. |
...
| promotion-service | 8096 | PostgreSQL `promotion_db`; Kafka | user wallet + admin/shop campaign; calculate/reservation internal | V2 durable voucher reservation, V3 deterministic outbox, V4 `promotion_order_reservation_receipts` atomic event-id/source/action/order/reservation/SHA-256 fence, locked wallet/capacity, 15-minute expiry, exact replay/conflict, commit/release and `COMMITTED -> RELEASED` compensation complete. Stacking supports one shop, platform, and freeship layer; shop vouchers require SHOP_OWNER creation plus ADMIN approval. Two Kafka/PostgreSQL replicas across duplicate partitions plus same/fresh group replay for commit and both release sources converge to one expected reservation/receipt/outbox transition; contradictory reuse goes directly to owner `.promotion.DLT`. Stacking/checkout/relay flags remain false by default and canary by stable principal. Flash-sale has its independent default-off receipt boundary. |
...
Exact method inventory hiện có tại `http-api-inventory.md`: **213 handler
...
|---|---|---|---|
| `order.created` | order | saga, notification, promotion, flashsale, restaurant inventory (gated); analytics disabled | active; immutable monetary and additive item snapshot (`orderItemId`, `menuItemId`, quantity, unit price/line total) plus nullable stable voucher/flash/inventory reservation identities. Notification atomically claims `notifications.deduplication_key = order-created:<eventId>` before optional FCM I/O; two Kafka consumers across duplicate partitions plus same/fresh-group replay converge to one stable row, while conflicting reuse goes directly to the owner `.notification.DLT`. The default-off Promotion reservation consumer atomically claims event ID/source/action/order/reservation/SHA-256 payload before commit, with two-replica same/fresh-group replay converging to one receipt and one committed reservation; contradictory reuse goes to `.promotion.DLT`. The default-off Flash-sale consumer atomically claims the same identity dimensions and SHA-256 before commit; its two-replica same/fresh-group Kafka/PostgreSQL rehearsal converges to one receipt/commit and directs contradictory reuse to `.flashsale.DLT`. Restaurant inventory claims `menu_item_inventory_order_receipts` before its idempotent commit; focused raw-event proof passes, while broker/PostgreSQL race rehearsal remains OPEN. Analytics, when enabled, claims the event before upserting `daily_item_sales`; legacy events without `items` remain compatible. |
| `order.cancelled` | order | saga, promotion, flashsale, restaurant inventory, settlement refund boundary (gated); analytics disabled | active core cancellation; carries event identity/occurredAt from the transactional outbox, nullable stable voucher/flash/inventory reservation identity, immutable monetary and additive persisted-item snapshot, plus typed `cancelledBySource`/`cancelReasonCode`. Customer/admin/restaurant cancellation, restaurant rejection, and payment failure enqueue the same compensation contract. Default-off Promotion and Flash-sale use atomic receipts before release; Restaurant inventory release is replay-fenced by event ID and restores committed stock, with focused proof only. When Analytics is enabled, the same event-level receipt fences the additive cancelled item projection. If explicitly enabled, Settlement atomically claims a refund case by event/idempotency/order-trigger before evaluating existing manual-review policy; exact replay no-ops and contradictory raw reuse goes to same-partition DLT. Provider/manual review remains fail-closed and default-off. |
| `order.refund-eligible` | order | promotion, flashsale, restaurant inventory, settlement refund boundary (gated) | active compensation trigger; produced when no-shipper matching exhausts before pickup; carries stable outbox event identity, `SHIPPER_NOT_FOUND` terminal status, typed `SYSTEM`/`SHIPPER_NOT_FOUND` source/reason, reservation IDs and the same immutable monetary snapshot. It never rewrites Order/Delivery to `CANCELLED`; default-off Promotion and Flash-sale atomically receipt the event before release, while Restaurant inventory uses the same replay-fenced release boundary. Settlement case creation remains atomically replay-safe and gated. |
...
| `shipper.found` batch variant | match | delivery, saga | additive khi capability bật; `batchOffer=true`, `batchId`, tối đa 3 `batchItems`, mỗi item giữ `matchingSessionId`, và COD hold IDs đầy đủ. Delivery persist batch aggregate/item rows atomically before per-order `delivery.offer-persisted`; shipper accept/reject là all-or-nothing. |
...
| `matching.decision-trace` | match | simulator-service (read-only, dev/test only) | active observability contract; Match ghi versioned `MATCHING_DECISION_TRACE` sau khi business result đã durable, với profile active `nearest-cod/v1` hoặc canary `balanced-eta/v1`, stage/total latency, attempts, candidates, `completedDeliveries`, score và rejection reasons. `balanced-eta` chỉ active khi operator bật non-zero canary; mặc định fail-closed. Outbox key `orderId`; simulator dùng consumer group riêng và không có business retry/DLT path. Trace mất hoặc malformed không được thay đổi reservation/assignment/Saga state; candidate list là post-GEO filter, không phải raw Redis pool. |
...
| `delivery.batch.accepted` | delivery | settlement | additive khi batch bật; durable batch-level COD `COMMITTED` intent chứa toàn bộ hold IDs, idempotent theo batch event ID; Delivery rows chuyển `WAIT_SHIPPER_CONFIRM -> ASSIGNED` atomically và phát BUSY projection cho từng item |
| `delivery.batch.released` | delivery | settlement, match | additive; expiry/reject hoặc pre-pickup whole-batch cancellation releases every COD hold and retires the old Match generation; payload carries aligned `deliveryIds` + `matchingSessionIds` so a rematch cannot retire a newer generation |
| `delivery.batch.completed` | delivery | match | additive; emitted once when every batch item reaches `DELIVERED`, retiring the durable Match pool generation and clearing old completion-time projection without touching already-consumed COD holds |
...
| `delivery.exception.reported` | delivery | settlement review bridge | gated/default-off; Delivery persists one exception per delivery, grants exactly one 15-minute retry after pickup, and moves expired/used cases to `RETURNING` then restaurant-confirmed `RETURNED`. The dedicated topic carries immutable post-pickup identity and canonical money snapshot; `RETURNING/RETURNED` are never sent through legacy `delivery.status-updated` because old Saga consumers reject unknown vocabulary. Settlement consumes only the initial `DELIVERY_EXCEPTION_REPORTED/RETRY_AVAILABLE` fact and creates an idempotent `DELIVERY_DISPUTE + ORDER_TOTAL` `MANUAL_REVIEW` case with no provider outbox. Unit/H2 proof PASS 2026-08-23; Kafka/PostgreSQL runtime and provider proof OPEN. |
...
| Restaurant menu inventory `order.created` | Order `eventId`, source topic/action/order/inventory reservation | `menu_item_inventory_order_receipts` claims source/action/order/reservation/SHA-256 before the idempotent Restaurant inventory commit; exact replay ACKs without another counter transition and contradictory reuse fails closed | Focused raw-event processor proof PASS 2026-08-23; PostgreSQL two-replica/Kafka replay and runtime expiry rehearsal remain OPEN; capability flags false by default. |
| Restaurant menu inventory `order.cancelled` / `order.refund-eligible` | Order `eventId`, source topic/action/order/inventory reservation | Same receipt fence classifies RELEASE; `COMMITTED -> RELEASED` restores on-hand quantity, while `RESERVED -> RELEASED/EXPIRED` returns held capacity | Focused service + processor proof PASS 2026-08-23; broker/runtime race and operational reconciliation remain OPEN. |
